...
def make_env(args, height, width, obs_shape):
...
def get_common_dmc_kwargs(args, env):
...
def main():
    ...
    env = make_env(args, input_obs_shape[0], input_obs_shape[1], input_obs_shape)
    test_env = make_env(args, input_obs_shape[0], input_obs_shape[1], input_obs_shape)

    # see Table 3 of CURL paper
    lr_ae = 2e-4 if args.env_name == "cheetah" else 1e-3
